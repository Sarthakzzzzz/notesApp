...
public class UserController {
    ...
    public ResponseEntity<User> getProfile(@RequestHeader("X-User-Id") Long userId) {
        return userRepository.findById(getCurrentUserId(userId)).map(user->ResponseEntity.ok(user)).orElse(ResponseEntity.notFound().build());
    }

    // GET/me -> update profile 
    @PutMapping
    public ResponseEntity<User> updateProfile(@RequestHeader("X-User-Id") Long userId, @RequestBody User updatedUser) {
        Optional<User> optionalUser =   userRepository.findById(getCurrentUserId(userId));
