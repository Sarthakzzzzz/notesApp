...
function SystemStatus({ user }) {
  ...
  const checkSystemStatus = async () => {
    ...
    try {
      ...
      newStatus.jwt = currentUser && authService.isAuthenticated() ? 'working' : 'failed';
    ...
    try {
      ...
      newStatus.backend = 'working';
      newStatus.auth = 'working';
  ...
  const getStatusText = (statusValue) => {
    switch (statusValue) {
      ...
      case 'no-user': return 'ℹ️ No User Context';
    ...
    <div className="app-content">
      <div className="section-header">
        <div>
          ...
          <p className="section-subtitle">Monitor system health and API endpoints</p>
        </div>
        <button 
      ...
      <div style={{ display: 'grid', gap: '1.5rem', marginBottom: '2rem' }}>
        {Object.entries(status).map(([key, value]) => (
          ...
          >
            <div>
              <h3 style={{ color: '#2d3748', fontWeight: '700', marginBottom: '0.5rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                {key}
              </h3>
            ...
            <div style={{ color: getStatusColor(value), fontWeight: '700', fontSize: '1rem' }}>
              {getStatusText(value)}
            </div>
          </div>
        ))}
      ...
      <div className="card" style={{ background: 'rgba(255, 255, 255, 0.95)', padding: '2rem', borderRadius: '1rem' }}>
        <h3 style={{ color: '#2d3748', fontWeight: '700', marginBottom: '1rem' }}>Current User Info</h3>
        <pre style={{ 
          background: '#f8fafc', 
          padding: '1rem', 
          borderRadius: '0.5rem', 
